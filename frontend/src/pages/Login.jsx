...
export default function Login(){
  ...
  const onSubmit = async (e)=>{
    ...
    catch(err){ setError(err?.response?.data?.detail || 'Usuario o contraseña inválidos') }
  ...
  return (
    <div style={{minHeight:'100vh', display:'grid', placeItems:'center',
      ...
      <div className="card" style={{width:'min(480px, 92vw)', padding:24, borderRadius:16, boxShadow:'0 20px 80px rgba(0,0,0,.2)'}}>
        ...
        <form onSubmit={onSubmit} className="grid" style={{marginTop:12}}>
          ...
          <button className="primary" disabled={busy} style={{display:'flex', alignItems:'center', gap:8, justifyContent:'center'}}>
            {busy && <span className="spinner" style={{ width:14, height:14, border:'2px solid var(--primary-ink)', borderTopColor:'transparent', borderRadius:'50%', animation:'spin .8s linear infinite' }}/>}
